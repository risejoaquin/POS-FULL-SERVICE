        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            if (decimal.TryParse(DiscountBox.Text, out decimal discount))
            {
                ...
                if (discount > 0)
                {
                    ...
                    {
                        var localAuthService = Microsoft.Extensions.DependencyInjection.ServiceProviderServiceExtensions.GetRequiredService<ILocalAuthService>(App.ServiceProvider!);
                        var overrideWindow = new ManagerOverrideWindow($"Aplicar descuento de {discount:C}", localAuthService);
