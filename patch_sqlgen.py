...
# Update signature
content = re.sub(r'public static string GenerateTenantSql\(.*?\)', 'public static string GenerateTenantSql(PosBuilder.Models.ConfigModel model)', content)
...
content = re.sub(r'return \$@"-- Script generado de ejemplo.*?";', sql_body, content, flags=re.DOTALL)

with open('PosBuilder/SqlGenerator.cs', 'w', encoding='utf-8') as f:
    f.write(content)

print("SqlGenerator updated")
