...
public class MainViewModelTests
{
    ...
    public void AddToCart_ShouldIncreaseQuantity_WhenProductAlreadyExists()
    {
        ...
        var mockApiService = new Mock<PosApplication.Interfaces.Local.IApiService>();
        var settings = Options.Create(new AppSettings());
        
        var serviceProviderMock = new Mock<IServiceProvider>();
        serviceProviderMock.Setup(x => x.GetService(typeof(PosDbContext))).Returns(dbContext);
        serviceProviderMock.Setup(x => x.GetService(typeof(PosApplication.Interfaces.Local.IApiService))).Returns(mockApiService.Object);
        ...
        var inventoryAppService = new Moq.Mock<PosApplication.Interfaces.Local.IInventoryAppService>();
        inventoryAppService.Setup(s => s.GetAllProductsAsync()).ReturnsAsync(new List<Product>());
        var localOrderService = new Moq.Mock<PosApplication.Interfaces.Local.ILocalOrderService>();
        var viewModel = new MainViewModel(settings, syncService, ticketPrinter, sessionManager, inventoryAppService.Object, localOrderService.Object);
    ...
    [Fact]
    public void ApplyDiscount_ShouldReduceTotal_ByTenPercent()
    {
        // Arrange
        var sessionManager = new SessionManager { CurrentTenantId = "test", Role = "Admin" };
        var dbContext = GetInMemoryDbContext(sessionManager);
        var mockApiService = new Mock<PosApplication.Interfaces.Local.IApiService>();
        var settings = Options.Create(new AppSettings());
        ...
        // Assert
        viewModel.IsDiscountApplied.Should().BeTrue();
        viewModel.DiscountAmount.Should().Be(10m); // 10% of 100
        viewModel.Total.Should().Be(90m); // 100 - 10
    }
}
