...
namespace PosBuilder
{
    public static class SqlGenerator
    {
        public static string GenerateTenantSql(PosBuilder.Models.ConfigModel model)
        {
            ...
            sb.AppendLine("-- 3.1 Insumos y Recetas (BOM)");
            sb.AppendLine("CREATE TABLE IF NOT EXISTS \"Supplies\" (\"Id\" SERIAL PRIMARY KEY, \"Name\" TEXT NOT NULL, \"UnitOfMeasure\" TEXT NOT NULL DEFAULT 'kg', \"Cost\" NUMERIC(10,2) NOT NULL DEFAULT 0.00 CHECK (\"Cost\" >= 0.00), \"Stock\" NUMERIC(10,2) NOT NULL DEFAULT 0.00, \"MinStockThreshold\" NUMERIC(10,2) NOT NULL DEFAULT 0.00, \"TenantId\" TEXT NOT NULL REFERENCES \"Tenants\"(\"Id\") ON DELETE CASCADE, CONSTRAINT \"PK_Supplies_Tenant_Id\" UNIQUE (\"Id\", \"TenantId\"));");
            sb.AppendLine("CREATE TABLE IF NOT EXISTS \"RecipeItems\" (\"Id\" SERIAL PRIMARY KEY, \"ProductId\" INTEGER NOT NULL, \"SupplyId\" INTEGER NOT NULL, \"Quantity\" NUMERIC(10,3) NOT NULL CHECK (\"Quantity\" > 0), \"TenantId\" TEXT NOT NULL REFERENCES \"Tenants\"(\"Id\") ON DELETE CASCADE, CONSTRAINT \"PK_RecipeItems_Tenant_Id\" UNIQUE (\"Id\", \"TenantId\"), CONSTRAINT \"FK_RecipeItems_Product\" FOREIGN KEY (\"ProductId\", \"TenantId\") REFERENCES \"Products\"(\"Id\", \"TenantId\") ON DELETE CASCADE, CONSTRAINT \"FK_RecipeItems_Supply\" FOREIGN KEY (\"SupplyId\", \"TenantId\") REFERENCES \"Supplies\"(\"Id\", \"TenantId\") ON DELETE CASCADE);");
